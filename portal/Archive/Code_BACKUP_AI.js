...
function resolveRoute(pageId, currentUrl) {
  ...
  // line_dashboard は常に SD-WAN 横並び資料を表示（ROUTE_MAP より優先）
  if (pageId === 'line_dashboard') {
    templateName = 'project_sd_wan';
    title = 'SD-WAN Comparison';
  } else {
  // Basic Routing Map
  const ROUTE_MAP = {
  ...
  // 3. Render
  try {
    if (templateName === 'project_sd_wan') return serveProjectSdWan();
    if (templateName === 'index') return serveIndexRaw(currentUrl);
    // 通常ブラウザでの白画面対策: 全テンプレートを raw+CSS で配信（createAccessibleHtml 未使用）
    try {
      return serveRawWithCss(templateName, title);
    } catch (rawErr) {
      return createAccessibleHtml(templateName, { url: currentUrl, projectId: pageId }, title);
    }
  } catch (renderErr) {
    console.warn(`Template '${templateName}' failed: ${renderErr.message}`);
    // project_sd_wan 失敗時はエラー内容を表示して原因切り分け
    if (templateName === 'project_sd_wan') {
      return renderErrorPage(new Error(`project_sd_wan 読み込み失敗: ${renderErr.message}`));
    }
...
/**
 * 指定 HTML を raw で返し、CSS インライン化のみ適用（テンプレート評価・リンク書き換えなし）。
 * ボタン遷移時の真っ白対策で index / landing 等に使用。
 */
function serveRawWithCss(filename, pageTitle) {
  ...
  // Chrome/Edge 対策: 外部 Tailwind CDN と Google Fonts を除去（GAS サンドボックスでブロックされ白画面になるため）
  ...
  // 遷移先の白画面対策: 絶対URLで正しい exec に遷移（target=_top は Drive で「ファイルを開けません」になるため付けない）
  const base = (ScriptApp.getService().getUrl() || '').replace(/\?.*$/, '');
  if (base) {
    html = html.replace(/href=["'](?!https?:\/\/)([^"']+)\.html["']/gi, 'href="' + base + '?page=$1"');
    html = html.replace(/href=["']\?page=([^"']+)["']/g, 'href="' + base + '?page=$1"');
  } else {
